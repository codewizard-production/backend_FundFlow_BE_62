package com.app.FundFlow_BE.nameBuilder;
...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "FundingRound".equals(entityTypeName) ? "FundingRounds" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Stage".equals(entityTypeName) ? "Stages" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Document".equals(entityTypeName) ? "Documents" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Investor".equals(entityTypeName) ? "Investors" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Founder".equals(entityTypeName) ? "Founders" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Startup".equals(entityTypeName) ? "Startups" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
